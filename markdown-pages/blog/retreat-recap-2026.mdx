...
This should make the packages easier to maintain, faster to compile, and more practical to adopt incrementally.
...
- Jaap Frolich: Rewriting the ReScript compiler in Rust / comptime evaluation
- Woonki Moon: ReScript Comptime PoC
- Paul Tsnobiladzé: ReScript-Shadcn
- Bernardo Gurgel: Signals all the way down
...
The compiler work from the retreat is now available in `13.0.0-alpha.4`.
...
- Added feature-gated source directories, so packages can include optional slices of their source tree at build time.
...
Finally, thanks to OpenAI for the generous "Codex for OSS" sponsorship.
