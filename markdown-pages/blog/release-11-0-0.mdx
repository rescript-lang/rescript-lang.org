...
- [Better Interop with Customizable Variants](./improving-interop.mdx)
...
- Blogpost: [Enhanced ergonomics for record types](./enhanced-ergonomics-for-record-types.mdx)
...
- [First-class Dynamic Import Support](./first-class-dynamic-import-support.mdx)
...
- Blogpost: [Uncurried mode](./uncurried-mode.mdx)
...
The `:>` (type coercion) operator now supports a few more datatypes.
...
Going forward, `create-rescript-app` is the new recommended way to setup a new project. It can even be used to quickly add ReScript to an existing project. Have a look at the updated [installation instructions](../docs/manual/installation.mdx) for how to use it.

### ReScript-React v0.12

In tandem with the new ReScript release, a new version of ReScript-React is released as well. It brings support for dynamically loading React components via `React.lazy`, and more. Check out the [corresponding docs](../docs/react/installation.mdx).
