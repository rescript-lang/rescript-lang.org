...
In ReactJS, props are usually described as a single `props` record. In ReScript, we use [labeled arguments](../../../docs/manual/function.mdx#labeled-arguments) to define our props parameters instead. Here's an example:
...
**Note:** React components are capitalized; primitive DOM elements like `div` or `button` are uncapitalized. More infos on the JSX specifics and code transformations can be found in our [JSX language manual section](/docs/manual/jsx#capitalized-tag).
