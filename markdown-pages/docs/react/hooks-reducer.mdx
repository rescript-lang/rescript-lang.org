...
**Note:** You will notice that the action / reducer pattern works especially well in ReScript due to its [immutable records](/docs/manual/record), [variants](/docs/manual/variant) and [pattern matching](/docs/manual/pattern-matching-destructuring) features for easy expression of your action and state transitions.
