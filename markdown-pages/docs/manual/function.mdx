...
When ReScript can evaluate a known pure function call ahead of time, it can emit the resulting data directly:
...
If all the arguments are known up front, ReScript can precompute the result too:
...
export { greet, callSecond, callFirst };
```
...
export { greet, getUserName, greetUser };
```
...
let SomeReScriptException = /* @__PURE__ */ Primitive_exceptions.create(
  "_tempFile.SomeReScriptException",
);

async function somethingThatMightThrow() {
  throw {
    RE_EXN_ID: SomeReScriptException,
    Error: new Error(),
  };
}
...
async function someAsyncFn() {
  let data;
  try {
    data = await somethingThatMightThrow();
  } catch (raw_exn) {
  ...
  return Primitive_option.some(data);
}

export { greet, SomeReScriptException, somethingThatMightThrow, someAsyncFn };
```
