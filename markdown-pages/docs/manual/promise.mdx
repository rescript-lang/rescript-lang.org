...
You can handle a rejected promise using the [`Promise.catch()`](./api/stdlib/promise#value-catch) method, which allows you to catch and manage errors effectively.
...
> **Note:** The `Js.Promise` bindings are following the outdated data-last convention from a few years ago. We kept those APIs for backwards compatibility. Either use [`Promise`](api/stdlib/promise) or a third-party promise binding instead.
