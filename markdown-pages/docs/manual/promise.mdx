...
You can handle a rejected promise using the [`Promise.catch()`](https://rescript-lang.org/docs/manual/api/latest/stdlib/promise#value-catch) method, which allows you to catch and manage errors effectively.
...
> **Note:** The `Js.Promise` bindings are following the outdated data-last convention from a few years ago. We kept those APIs for backwards compatibility. Either use [`Promise`](https://rescript-lang.org/docs/manual/api/latest/stdlib/promise) or a third-party promise binding instead.

ReScript has built-in support for [JavaScript promises](https://rescript-lang.org/docs/manual/api/latest/js/promise). The 3 functions you generally need are:
