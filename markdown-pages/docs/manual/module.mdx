...
export { BaseComponent, ActualComponent };
```
...
signature declares, just like the module `School` defined above (if we
...
function MakeSet(Item) {
  let add = (currentSet, newItem) => {
    if (Stdlib_List.some(currentSet, (x) => Item.equal(x, newItem))) {
      return currentSet;
    } else {
      return {
        hd: newItem,
...
function add(currentSet, newItem) {
  if (Stdlib_List.some(currentSet, (x) => equal(x, newItem))) {
    return currentSet;
  } else {
    return {
      hd: newItem,
...
export { MakeSet, IntPair, SetOfIntPairs };
```
