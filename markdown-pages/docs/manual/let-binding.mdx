...
canonical: "/docs/manual/let-binding"
...
export { greeting, score, newScore };
```

</CodeTab>

Because these bindings are pure and known up front, the JS output can inline the calculation and emit `20` directly for `newScore`.
...
The whole block is pure too, so the generated JS can collapse it to the final string literal.
...
If you need _real_ mutation, e.g. passing a value around, have it modified by many pieces of code, we provide a slightly heavier [mutation feature](./mutation.mdx).

## Private let bindings

Private let bindings are introduced in the release [7.2](../../blog/archived/bucklescript-release-7-2.mdx).
