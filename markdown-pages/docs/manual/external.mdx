...
We've specified `document`'s type as `'a`, a placeholder type that's polymorphic. Any value can be passed there, so you're not getting much type safety (except the inferences at various call sites). This is excellent for quickly getting started using a JavaScript library in ReScript **without needing a separate repository of typed bindings** -- you can write bindings directly for any API you need.
