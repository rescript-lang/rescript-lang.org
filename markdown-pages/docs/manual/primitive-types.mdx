...
See the familiar `String` API in the [API docs](https://rescript-lang.org/docs/manual/api/latest/stdlib/string). Since a ReScript string maps to a JavaScript string, you can mix & match the string operations in all standard libraries.
...
A regular expression like the above has the type `RegExp.t`. The [RegExp](https://rescript-lang.org/docs/manual/api/latest/stdlib/regexp) module contains the regular expression helpers you have seen in JS.
...
32-bits, truncated when necessary. We provide the usual operations on them: `+`, `-`, `*`, `/`, etc. See [Int](https://rescript-lang.org/docs/manual/api/latest/stdlib/int) for helper functions.
...
Float requires other operators: `+.`, `-.`, `*.`, `/.`, etc. Like `0.5 +. 0.6`. See [Float](https://rescript-lang.org/docs/manual/api/latest/stdlib/float) for helper functions.
...
We provide the usual operations on them: `+`, `-`, `*`, `/`, etc. See [BigInt](https://rescript-lang.org/docs/manual/api/latest/stdlib/bigint) for helper functions.
