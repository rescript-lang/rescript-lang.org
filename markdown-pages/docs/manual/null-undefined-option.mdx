...
To solve this, we provide access to more elaborate `null` and `undefined` helpers through the [`Nullable`](https://rescript-lang.org/docs/manual/api/latest/stdlib/nullable) module. This somewhat works like an `option` type, but is different from it.
