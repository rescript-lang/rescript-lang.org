...
A variant allows a type to convey that it can be one of many possible values. [TypeScript](https://www.typescriptlang.org/docs/handbook/enums.html) and [Rust's](https://doc.rust-lang.org/book/ch06-01-defining-an-enum.html) have a similar concept with their `enum` types.

A variant type uses `|` to separate the possible cases, which are called variant constructors. Each constructor must start with a capital letter.
...
`myResponse` is a variant type with the cases `Yes`, `No` and `PrettyMuch`.

A variant type should be used to express business logic and to represent that logic with the type system. Variant types can be used with [pattern matching](./pattern-matching-destructuring.mdx) that allows you to conveniently handle the possible cases, and to receive warnings if you forget to handle a case.

<CodeTab labels={["ReScript", "JS Output"]}>

```res example
type dbClient
type dbClientConnection =
  | Connected(dbClient)
...
let dbStatus = client => {
  switch client {
    | Connected(_) => "Database is connected"
    | ConnectionFailed(reason) => `Database connection failed with reason ${reason}`
    | ConnectionNotStarted => "Database connection has not yet started"
  }
}
```
...
A variant type is fixed when defined and cannot be changed later on, but there are two other types of variants that you can use if you need a more dynamic variant type. [Extensible variants](./extensible-variant.mdx) allow you to create a type and add the variant constructors later, and [polymorphic variants](./polymorphic-variant.mdx) can be dynamically created without the need to define anything at the start.

- Use a variant type when you know the possible variants and want to enforce exhaustive pattern matching from a single type definition. This is what you will use most of the time.
- Use polymorphic variants when you don't need to define the types and want to have a very dynamic variant that can be very different from one place to another. This is useful for collecting possible error types with [`result`](/docs/manual/api/stdlib/result).
- Use extensible variants when you don't fully know the data and want to extend and are OK with non-exhaustive pattern matching. You will rarely need this, but it's a powerful advanced feature.
...
The output is slightly uglier and less performant than the former, but it can be used if you want to use the type outside of the variant.
...
## Using variants in different files

If the variant you're using is in a different file, you'll need to use the module name with an explicit type annotation.
...
With _untagged variants_ it is possible to mix types together that normally can't be mixed in the ReScript type system, as long as there's a way to discriminate them at runtime. For example, with untagged variants you can represent a heterogeneous array:
...
So, we'll need to go back to `string`, losing all of the goodies of using a variant, and then do manual conversion into the `animal` variant from `string`, right?
