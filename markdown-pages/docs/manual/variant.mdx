...
A variant type uses `|` to separete the possible cases, which are called variant constructors. Each constructor must start with a capital letter.
...
A variant type should be used to express business logic and to represent that logic with the type system. Variant types can be used with (patern matching)[./pattern-matching-destructuring.mdx] that allows you to convientently handle the possible cases, and to revieve warnings it you forget to handle a case.
...
- Use polymorphic variants when you don't need to define the types and want to have a vary dynamic variant that can be very different from one place to another. This is useful for collecting possible error types with (`result`)[/docs/manual/api/stdlib/result].
