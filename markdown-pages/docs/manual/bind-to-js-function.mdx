...
let firstItem = myArray$1[0];

let tenthItem = myArray$1[10];
...
let message = myList !== 0 ? (
    myList.tl !== 0 ? "The list two items and a potenial remainder of items" : "The head of the list is the string " + (1).toString()
  ) : "This list is empty";
