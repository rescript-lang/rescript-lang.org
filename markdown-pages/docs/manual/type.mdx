...
Note: in the last line, `(~radius as r: int)` is a labeled argument. More on this in the [function](function) page.
...
**Note**: this particular `external` is the only one that isn't preceded by a `@` [attribute](attribute).
