...
- Sound by design -- types are always correct. If a type isn't marked as nullable, the value is guaranteed to never be `undefined`. **ReScript code has no null/undefined errors**.
- Consistent across all projects -- the type system has a single, well-defined behavior with no configuration needed.
