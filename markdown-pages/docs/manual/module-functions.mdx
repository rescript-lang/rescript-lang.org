...
canonical: "/docs/manual/module-functions"
...
function _tempFile$Component(props) {
  let params = Navigation.useParams();
...
function MakeDbConnection(Config) {
  let client = DatabaseJs.makeClient(Config.host, Config.database, Config.username, Config.password);
  return {
    client: client
  };
}

let client = DatabaseJs.makeClient("localhost", "mydb", "root", "password");

let Db = {
  client: client
};

let updateDb = client.write("new value");
