...
external elementToDialog: DomTypes.element => DomTypes.htmlDialogElement = "%identity"

let getMobileOverlayDialog = () => {
  document
  ->Document.getElementById("mobile-overlay")
  ->Null.map(elementToDialog)
  ->Null.asNullable
}

@get external _open: DomTypes.htmlDialogElement => bool = "open"

let openMobileOverlay = _ =>
  switch getMobileOverlayDialog() {
  | Nullable.Value(dialog) => dialog->HTMLDialogElement.showModal
  | Null => ()
  | Undefined => ()
  }
...
let closeMobileOverlay = _ =>
  switch getMobileOverlayDialog() {
  | Nullable.Value(dialog) => dialog->HTMLDialogElement.close
  | Null => ()
  | Undefined => ()
  }
...
let getMobileTertiaryDialog = () => {
  document
  ->Document.getElementById("mobile-tertiary-drawer")
  ->Null.map(elementToDialog)
  ->Null.asNullable
}
