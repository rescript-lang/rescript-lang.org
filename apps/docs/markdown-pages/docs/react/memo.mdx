...
let make = React.memo((props) =>
  JsxRuntime.jsxs("div", {
    children: [props.a, JsxRuntime.jsx("br", {}), props.b],
  }),
);
...
function Example(props) {
  ...
  return JsxRuntime.jsx("button", {
    ...
    onClick: (ev) => onClick((ev.preventDefault(), undefined)),
  });
}

let make = React.memo(Example, (p1, p2) => p1.disabled === p2.disabled);
...
Another approach is to use a custom prop type and wrap the component function with `React.component`.
...
let make = React.memoCustomCompareProps(
  React.component(({disabled, onClick}: props) => {
...
let make = React.memo(
  (param) => {
    let onClick = param.onClick;
    return (
      <button disabled={param.disabled} onClick={(ev) => onClick(ev)}>
        {"My button"}
      </button>
    );
  },
