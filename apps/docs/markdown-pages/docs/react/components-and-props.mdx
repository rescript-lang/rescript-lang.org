...
function Example(props) {
  return <div>{"Hello ReScripters!"}</div>;
}
...
function Example(props) {
  let visitorCountMsg = "You are visitor number: " + String(props.visitorCount);
...
function Example(props) {
  let name = props.name;
  let greeting = name !== undefined ? "Hello " + name + "!" : "Hello stranger!";
  return JsxRuntime.jsx("div", {
    children: greeting,
  });
}
...
function Example$Greeting(props) {
  return Stdlib_Option.getOr(props.name, "");
}
...
JsxRuntime.jsx(Example$Greeting, {
  name: name,
});
...
JsxRuntime.jsxs(Example$MyList, {
  children: [
    JsxRuntime.jsx("li", {
      children: "Item 1",
    }),
...
function Example(props) {
  return JsxRuntime.jsx("div", {
    children: JsxRuntime.jsx(Example$Greeting, {}),
  });
}
...
### Components with a Props Record

Use `@react.componentWithProps` when a component takes a single props record instead of labeled arguments.
...
<Link.make href={"/docs"}>{"Docs"}</Link.make>;
