...
- [`taggedTemplate`](./bind-to-js-function.mdx#tagged-template-functions)
...
let result2 = Stdlib_Nullable.fromOption(10);
...
let result3 = Primitive_option.fromNullable(10);

export { jsNull, jsUndefined, result1, result2, result3 };
```
...
```js
console.log([1, 2, 3].map((a) => (a + 1) | 0).filter((a) => a % 2 === 0));
```
