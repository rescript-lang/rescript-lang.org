...
let expensiveFilesRead = Stdlib_Lazy.make(() => {
  console.log("Reading dir");
  return Nodefs.readdirSync("./pages");
});

console.log(Stdlib_Lazy.get(expensiveFilesRead));

console.log(Stdlib_Lazy.get(expensiveFilesRead));

export { expensiveFilesRead };
```
...
try {
  result = Stdlib_Lazy.get(expensiveFilesRead);
} catch (raw_exn) {
...
export { expensiveFilesRead, result };
```
