...
export { student1, name, myResult, message };
```
...
if (person1.TAG === "Teacher") {
  ...
  let match = "Jane";
  let match$1 = match.status;
  let name = match.name;
  let match$2 = match.reportCard;
  if (match$2.passing) {
    message =
      `Congrats ` +
      name +
...
export { person1, message, myStatus };
```
...
export { person1, message, myStatus, greetPet, greetFish, greetAnimal };
```
...
export { person1, message, myStatus, isPet };
```
...
export { person1, message, myStatus, students };
```
...
function printStudents(_students) {
  while (true) {
    ...
    let otherStudents = students.tl;
    let student = students.hd;
    if (otherStudents !== 0) {
      console.log(student);
...
export { person1, message, myStatus, printStudents };
```
...
export { person1, message, myStatus, d, b, coordinates, centerY };
```
...
let myNullableValue = 5;

export { person1, message, myStatus, d, b, myNullableValue };
```
...
export { person1, message, myStatus, d, b, optionBoolToBool };
```
