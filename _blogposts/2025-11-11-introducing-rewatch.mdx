...
For projects that need it, the legacy bsb build system remains available throughout the v12 release cycle through the `rescript-legacy` command. This is a separate binary, not a subcommand. We expect to remove the legacy build system in v13, so we encourage migrating to ReWatch when possible.
