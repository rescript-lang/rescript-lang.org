...
date: "2025-11-11"
badge: roadmap
title: "A Smarter Build System for ReScript"
description: |
...
1. The build system scans your ReScript source files
...
The build system uses CMI files to make intelligent decisions about what needs recompiling. Here's how:

When you change a file, the build system:
...
If the hashes match, the module's public interface hasn't changed, and dependent modules can skip recompilation.
...
This is the key innovation. The new system doesn't just track which modules depend on each other. It understands when those dependencies actually matter.
...
The new build system represents a significant leap forward for ReScript's build tooling. The intelligent dependency tracking means less waiting and more building. The proper monorepo support means it scales with your project structure. The performance improvements are most noticeable where they matter most: during iterative development.

These improvements come from fundamental changes in how the build system understands and processes your code. By tracking interface stability rather than just file changes, it does less work while being smarter about what actually needs rebuilding.
...
If you're upgrading to ReScript 12, you'll get the new build system automatically. We're excited to hear how it works for your projects. As always, feedback and bug reports are welcome. You can reach us through the forum or on GitHub.
